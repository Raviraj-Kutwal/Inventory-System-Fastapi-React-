...
import "../App.css"; // Go up one level since this is in components/
...
function Inventory() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (sortField === "id" || sortField === "price" || sortField === "quantaity") {
        aVal = Number(aVal);
        bVal = Number(bVal);
      } else {
  ...
  // Reset form
  const resetForm = () => {
    setForm({ id: "", name: "", description: "", price: "", quantaity: "" });
    setEditId(null);
  };
    ...
    try {
      if (editId) {
        await api.put(`/products/${editId}`, {
          ...
          quantaity: Number(form.quantaity),
        });
                ...
                <table className="product-table">
                  <thead>
                    <tr>
                      ...
                      <th 
                        className={`sortable ${sortField === 'quantaity' ? `sort-${sortDirection}` : ''}`}
                        onClick={() => handleSort('quantaity')}
                      >
                        Quantity
                      </th>
                      <th>Actions</th>
                  ...
                  <tbody>
                    {filteredProducts.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td>
                          <span className="qty-badge">{p.quantaity}</span>
                        </td>
                        <td>
